...
export function ShareSourcesChart() {
  const [date, setDate] = useState<DateRange | undefined>({
    from: subDays(new Date(), 30),
    to: new Date(),
  });
  const [shareSources, setShareSources] = useState<{ name: string; value: number }[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const res = await fetch(`/api/analytics/demographics/sharesources?${params.toString()}`);
        const json = await res.json();
        if (json.shareSources) setShareSources(json.shareSources);
      } catch (error) {
        console.error("Error fetching share sources:", error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden shadow-sm flex flex-col h-full">
      ...
      <div className="p-4 flex-1 flex flex-col justify-center min-h-[250px]">
        {isLoading || shareSources.length > 0 ? (
          <div className="h-[250px] w-full flex items-center justify-center">
            <EvilLineChart
              data={shareSources as any}
          ...
          <p className="text-sm text-muted-foreground text-center my-12">No share source data recorded</p>
        )}
      </div>
    </div>
  );
